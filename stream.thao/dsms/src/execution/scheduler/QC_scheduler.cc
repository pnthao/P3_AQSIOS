...
void QCScheduler::updateNodeInfo(Communicator * comm){
	...
	//update the load of active queries
	for(int i=0;i<num_loadMgrs; i++){
		...
		for(int o=0;o<loadMgr->numOps;o++){
			if(loadMgr->ops[o]->kind == Physical::PO_OUTPUT && loadMgr->ops[o]->b_active ==true){
				...
				loadMgr->ops[o]->u.OUTPUT.cycleCount = 0;
			}
		}
...
void QCScheduler::updateActiveQueriesList(Execution::Communicator* comm){

	//the initial active query list, no other thread has been created at this point yet so
