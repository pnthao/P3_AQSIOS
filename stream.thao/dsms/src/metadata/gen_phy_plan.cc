									 ...
									 Physical::Operator *&phyPlan)
{
	...
	//notes by Thao Pham: there is no case for drop operator here, since the drop operator only appears at physical level	
	default:
		// unknown kind
		ASSERT (0);
		return -1;
...
//insert the drop operator to a specific point in the plan, by Thao Pham 
int PlanManagerImpl::mk_drop(Operator *child, Operator *&drop)
{
	int rc;
	drop = new_op(Physical::PO_DROP);
	
	if (!drop) {
		LOG << "PlanManager: no space for drop operators" << endl;
		return -1;
	}
	...
	//input of drop operator has the same schema as its child
	if ((rc = copy_schema (drop, child)) != 0)
		return rc;
	
	//now we expect output of child op to be stream
	ASSERT(child ->bStream);
	drop -> bStream = true;	
	drop -> numOutputs = 0;	
	drop -> numInputs = 1;
	...
	if ((rc = addOutput (child, drop)) != 0)
		return rc;
	
	drop->u.DROP.dropPercent = 0;
	//drop ->u.DROP.active = false;
...
Operator *PlanManagerImpl::new_op (OperatorKind kind)
{
	...
	op->b_active = false; // the op is inactive by default
