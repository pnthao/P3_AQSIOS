...
//output queue for drop operator, by Thao Pham
static int set_out_queue_drop (Operator *op, Execution::Queue *queue);
//end of output queue for drop operator, by Thao Pham
...
//input queue for drop operator, by Thao Pham
static int set_input_queue_drop (Operator *op, Execution::Queue *queue, unsigned int pos);
//end of input queue for drop operator, by Thao Pham
...
//in store for drop operator, by Thao Pham
static int set_in_store_drop (Operator *op);
//end of in store for drop operator, by Thao Pham
...
int PlanManagerImpl::inst_op (Operator *op) {
	...
	//call method to instantiate drop operator, by Thao Pham
	case PO_DROP: 
		if((rc = inst_drop(op))!=0)
			return rc;
		break;
	//end of calling method to instantiate drop operator, by Thao Pham
...
{	
	...
	//drop operator, by Thao Pham
	case PO_DROP:
		return set_out_queue_drop (op, queue);
	//end of drop operator, by Thao Pham	
...
{	
	...
	//drop operator, by Thao Pham
	case PO_DROP:
		return set_input_queue_drop (op, queue, inputPos);
	//end of drop operator, by Thao Pham
...
//set output queue for drop operator, by Thao Pham
	
static int set_out_queue_drop (Operator *op, Execution::Queue *queue)
{
	return ((Drop *)(op -> instOp)) -> setOutputQueue (queue);
}
//end of setting output queue for drop operator, by Thao Pham
...
//set input queue for drop operator, by Thao Pham
static int set_input_queue_drop (Operator *op, Execution::Queue *queue, unsigned int pos)
{
	ASSERT (pos == 0);
	return ((Drop *)(op -> instOp)) -> setInputQueue (queue);
}
//end of setting input queue for drop operator, by Thao Pham
...
//set in store for drop operator, by Thao Pham
static int set_in_store_drop (Operator *op)
{
	return ((Drop *)(op -> instOp)) -> setInStore(op -> inStores [0] -> instStore);
}
//end of setting in store for drop operator, by Thao Pham
