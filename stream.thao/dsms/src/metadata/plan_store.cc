									 ...
									 Store *store)
{
	...
	case PO_ROW_WIN:
		...
		for (unsigned int o = 0 ; o < op -> numOutputs ; o++) {
			...
			//notes by Thao Pham: the window operators do not have a store(they do not modify input tuples),
			// so they forward the store to their parents operators (up in the tree)
...
int PlanManagerImpl::add_store_select (Operator *op)
{
	...
	for (unsigned int o = 0 ; o < op -> numOutputs ; o++) {
		...
		outOp = op -> outputs [o];
		inputIdx [o] = getInputIndex (outOp, op); //op corresponds to which index of outOp...
...
int PlanManagerImpl::add_store (Operator *op)
{
	...
	//drop op, by Thao Pham
	case PO_DROP:
		//it should output stream
		ASSERT(op->bStream);
		
		storeKind = SIMPLE_STORE;
		
		op->u.DROP.kind = PO_LWDROP;
		
		for (unsigned int o = 0 ; o < op -> numOutputs ; o++) {
			if (bReqStore [o]) {
				storeKind = WIN_STORE;
				//this drop need to be a heavy-weight drop to have an output store for some of its parents
				op->u.DROP.kind = PO_HWDROP;
				break;
			}
		}
		...
		break;
	//end of drop op, by Thao Pham	
	default:
		ASSERT (0);
		break;
	};
	
	//by Thao Pham: light-weight drop does not have a store
	if(op->kind ==PO_DROP && op->u.DROP.kind==PO_LWDROP)
		return 0;
	//end of drop, by Thao Pham
...
static int set_in_stores_s (Operator *op)
{
	...
	for (unsigned int i = 0 ; i < op -> numInputs ; i++) {
		...
		//modify by Thao Pham for lightweight drop
		else {
			ASSERT (child -> kind == PO_SELECT    ||
					child -> kind == PO_RANGE_WIN ||
					child -> kind == PO_ROW_WIN||
					(child -> kind == PO_DROP && child->u.DROP.kind == PO_LWDROP));
		//end of modifying by Thao Pham for lightweight drop	
