							...
							Logical::Operator *& queryPlan) 
{
	...
	/*commented out by Thao Pham
	 *  this checking to remove IStream is not thorough, 
	 * since the source of the query may be a virtual query, and its input 
	 * may turn out to produce a relation rather than stream later on
	 */ 
	/*if((rc = t_removeIstream(queryPlan)) != 0)
		return rc;
	
	ASSERT (check_plan (queryPlan));*/
	//end of commenting out by Thao Pham
