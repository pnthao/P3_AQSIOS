...
// the command that the user uses in order to 
// specify the query class this query belongs to 
static const char *Q_CLASSES_COMMAND = "qclass";
...
//Query Class Scheduling by Lory Al Moakar 
	//capture if this command is a query class specification
	else if (strncmp (begin, Q_CLASSES_COMMAND, ptr - begin) == 0) {
		command.type = QCLASS;
	}
