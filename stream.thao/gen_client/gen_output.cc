...
int GenOutput::putNext (const char *tuple, unsigned int len, unsigned 
			long long int responseTime, int time_unit)
{
	Timestamp timestamp;
	int ival;
	float fval;
	...
	memcpy (&timestamp, tuple + tstampOffset, TIMESTAMP_SIZE);
	unsigned long long int adjusted_ts =(unsigned long long int) timestamp *
	  (unsigned long long int) time_unit;
	  //out << "[" << (long long int)(timestamp*time_unit) << "]";
	out << "[" <<  adjusted_ts  << "]";
	out << ":" << tuple [signOffset] << ":";
	...
	//output the response time in nsecs
	out << " : [" << responseTime <<"]";
	out << std::endl;
